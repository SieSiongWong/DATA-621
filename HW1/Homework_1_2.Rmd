...
Now, lets take a look at the correlation between TARGET_WINS and predictors. We can see from table below, the top correlated attributes with the response variable TARGET_WINS are base hits by batters and walks by batters. This can be used as one of the reference later for choosing models.

```{r}

complete_train_data %>% select(TARGET_WINS:TEAM_FIELDING_DP) %>% cor() %>% as.data.frame() %>% select(TARGET_WINS) %>% arrange(desc(TARGET_WINS))
