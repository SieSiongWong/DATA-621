...
training %>% select(-INDEX) %>% gather(variable, value) %>% 
...
We're imputing the missing data using the MICE package. The method of predictive mean matching (PMM) is selected for continuous variables. Before doing imputation, we drop the INDEX and TEAM_BATTING_HBP variables as these two variables are useless.
...
Now, we want to remove outliers from the dataset as many statistical parameters such as mean, standard deviation, correlation are very sensitive to outliers. One of the ways to locate outliers and eliminate is using Interquartile Range (IQR) method. In this method, a point is treated as an outlier if it's above the 75th or below the 25th percentile by a factor of 1.5 times the IQR.

After we've removed outliers, the boxplot looks much better compared to the original. Also, the distribution for some variables as shown in the histogram also got improved a lot, nearly normal.
...
lm2 <- lm(TARGET_WINS ~ ., data = subset(complete_train_data, select=-c(TEAM_BATTING_H,TEAM_BASERUN_CS,TEAM_PITCHING_HR,TEAM_PITCHING_SO,TEAM_PITCHING_BB)))
summary(lm2)
...
lm3 <- lm(TARGET_WINS ~ ., data = subset(complete_train_data, select=-c(TEAM_BATTING_H,TEAM_BASERUN_CS,TEAM_PITCHING_HR,TEAM_PITCHING_SO,TEAM_PITCHING_BB,TEAM_BASERUN_SB,TEAM_BATTING_3B)))
summary(lm3)
...
results_model_one = predict(lm,newdata = complete_train_data)
results_model_two = predict(lm2,newdata = complete_train_data)
results_model_three = predict(lm3,newdata = complete_train_data)

results_df = data.frame(complete_train_data$TARGET_WINS,results_model_one,results_model_two,results_model_three)

ggplot(results_df,aes(results_model_one,complete_train_data.TARGET_WINS)) + geom_point()+ geom_abline(intercept=0, slope=1) +
  labs(x='Predicted Values', y='Actual Values', title='Predicted vs. Actual Values')

ggplot(results_df,aes(results_model_two,complete_train_data.TARGET_WINS)) + geom_point()+ geom_abline(intercept=0, slope=1) +
  labs(x='Predicted Values', y='Actual Values', title='Predicted vs. Actual Values')

ggplot(results_df,aes(results_model_three,complete_train_data.TARGET_WINS)) + geom_point()+ geom_abline(intercept=0, slope=1) +
  labs(x='Predicted Values', y='Actual Values', title='Predicted vs. Actual Values')
