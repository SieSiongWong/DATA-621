...
# Residuals plot
plot(lm_2$fitted.values, lm_2$residuals, 
     xlab='Fitted Values', ylab='Residuals')
abline(h = 0, lty = 3, col="blue")
abline(h = 40, lty = 3, col="red")
abline(h = -40, lty = 3, col="red")
...
# Histogram plot
hist(lm_2$residuals, xlab="Residuals")
...
# Check for nonconstant variances: H0: Constant Variance, HA: Nonconstant Variance
# P-value <.05, reject null hypothesis for alternative indicating Nonconstant Variance
summary (lm(abs (lm_2$residuals) ~ lm_2$fitted.values))
...
# Residuals plot
plot(lm_3$fitted.values, lm_3$residuals, 
     xlab='Fitted Values', ylab='Residuals')
abline(h = 0, lty = 3, col="blue")
abline(h = 40, lty = 3, col="red")
abline(h = -40, lty = 3, col="red")
...
# Histogram plot
hist(lm_3$residuals, xlab="Residuals")
...
# Check for nonconstant variances: H0: Constant Variance, HA: Nonconstant Variance
# P-value <.05, reject null hypothesis for alternative indicating Nonconstant Variance
summary (lm(abs (lm_3$residuals) ~ lm_3$fitted.values))
...
Below, we will evaluate the models we have built using the Root Mean Square Error (RMSE) in order to determine the model with the smallest errors. 

We will also construct a visual representation of the errors in a histogram for each model to show how many of the predicted values are closer to an error of 0 in regards to the actual values.

```{r MarioEval1}
#This section should go before the models. Let's discuss possible variable names that will need to be used in the models above.

set.seed(123)
train_index <- createDataPartition(complete_train_data$wins, p = .7, list = FALSE, times = 1)
...
additional_test <- additional_test %>% mutate(lm_error = wins - lm)
...
```{r MarioEval3}
additional_test$lm_2 <- predict(lm_2, additional_test)
additional_test <- additional_test %>% mutate(lm_2_error = wins - lm_2)
ggplot(additional_test, aes(lm_2_error)) +
  geom_histogram(bins = 50, color = "black") + annotate("text",x=0,y=10, label = paste("RMSE = ", round(sqrt(mean(additional_test$lm_2_error^2)),2)),color="white")
```

```{r MarioEval4}
additional_test$lm_3 <- predict(lm_3, additional_test)
additional_test <- additional_test %>% mutate(lm_3_error = wins - lm_3)
ggplot(additional_test, aes(lm_3_error)) +
  geom_histogram(bins = 50, color = "black") + annotate("text",x=0,y=10, label = paste("RMSE = ", round(sqrt(mean(additional_test$lm_3_error^2)),2)),color="white")
```
